...
async function verifyAuthParsing() {
    logger.info(`[VERIFY] Fetching ${BASE_URL}/join ...`);
    try {
        ...
        if (!response.ok) {
            logger.error(`[FAIL] HTTP Error: ${response.status} ${response.statusText}`);
            return;
        }

        const html = await response.text();
        logger.info(`[SUCCESS] Fetched ${html.length} bytes.`);

        // 1. Verify CSRF Token Extraction
        logger.info('[VERIFY] Checking for CSRF Token...');
        const csrfMatch = html.match(/name="csrf-token" content="(.*?)"/) || html.match(/input type="hidden" name="csrf" value="(.*?)"/);
        const csrfToken = csrfMatch ? csrfMatch[1] : null;

        if (csrfToken) {
            logger.info(`[PASS] Found CSRF Token: ${csrfToken.substring(0, 15)}...`);
        } else {
            logger.error('[FAIL] Could not find CSRF Token! Login attempts will likely fail 401/403.');
            const headMatch = html.indexOf('<head>');
            const headEndMatch = html.indexOf('</head>');
            if (headMatch !== -1 && headEndMatch !== -1) {
                logger.info('HTML Preview of head:', html.substring(headMatch, Math.min(headEndMatch + 7, headMatch + 500)));
            }
        }

        // 2. Verify User ID Discovery (Hidden vs Visible)
        logger.info(`[VERIFY] Checking visibility of user "${TARGET_USERNAME}"...`);
        ...
        if (visibleMatch) {
            logger.info(`[INFO] User list is VISIBLE. Found ID: ${visibleMatch[1]}`);
        } else if (jsonMatch) {
            logger.info(`[INFO] User found in JSON data. Found ID: ${jsonMatch[1]}`);
        } else {
            logger.info(`[INFO] User "${TARGET_USERNAME}" NOT found in HTML. User list is likely HIDDEN.`);
            logger.info(`[PASS] Client should fallback to using username "${TARGET_USERNAME}" as ID.`);
        }

    } catch (e: any) {
        logger.error("[FAIL] Execution Error:", e.message);
    }
}
