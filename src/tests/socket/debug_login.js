...
async function debugLogin() {
    logger.info(`fetching ${BASE_URL}/join ...`);
    try {
        ...
        while ((match = inputRegex.exec(html)) !== null) {
            logger.info(`Found Input: Name="${match[1]}", Tag="${match[0]}"`);
        }

        // Also look for selects just in case
        logger.info('--- LOOKING FOR SELECTS ---');
        const selectRegex = /<select[^>]*name="([^"]+)"[^>]*>/g;
        while ((match = selectRegex.exec(html)) !== null) {
            logger.info(`Found Select: Name="${match[1]}", Tag="${match[0]}"`);
        }
